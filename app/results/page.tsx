      ...
      <main style={{ display: "flex", flexDirection: "column", alignItems: "center", minHeight: "100vh", padding: "0 0 120px", gap: "0" }}>
        ...
        <div style={{ width: "100%", maxWidth: "480px", display: "flex", flexDirection: "column", gap: "12px", padding: "20px 16px 0" }}>
          ...
          {result.identificationConfidence != null && (
            <FadeUp delay={0.2}>
              <div style={{ display: "flex", alignItems: "center", gap: "8px", padding: "0 4px" }}>
                <div style={{ width: "8px", height: "8px", borderRadius: "50%", background: (result.identificationConfidence || 0) >= 80 ? "var(--green-accent)" : (result.identificationConfidence || 0) >= 60 ? "#c9a84c" : "var(--red)" }} />
                <span style={{ fontSize: "12px", color: "var(--text-secondary)" }}>
                  {(result.identificationConfidence || 0) >= 80 ? "High" : (result.identificationConfidence || 0) >= 60 ? "Medium" : "Low"} confidence
            ...
            <FadeUp delay={0.25}>
              <div className="card">
                <div style={{ display: "flex", alignItems: "center", gap: "6px", marginBottom: "10px" }}>
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--green-accent)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 18h6M10 22h4M12 2a7 7 0 0 0-4 12.7V17h8v-2.3A7 7 0 0 0 12 2z"/></svg>
                  <p className="card-label" style={{ marginBottom: 0 }}>Selling Tips</p>
                </div>
                {result.tips.map((tip, i) => (
                  <p key={i} style={{ fontSize: "13px", color: "var(--text-secondary)", lineHeight: 1.5, padding: "4px 0", display: "flex", gap: "8px" }}>
          ...
          {result.interestingFact && (
            <FadeUp delay={0.3}>
              <div style={{ background: "var(--green-light)", border: "1px solid transparent", borderRadius: "14px", padding: "14px 18px" }}>
                <p style={{ fontSize: "12px", fontWeight: 600, color: "var(--green-accent)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: "4px" }}>Did you know?</p>
          ...
          {result.bestPlatform && result.platformReason && (
            <FadeUp delay={0.35}>
              <div className="card" style={{ borderColor: "var(--green-accent)", borderWidth: "1px" }}>
                <p className="card-label">Best Platform to Sell</p>
                <p style={{ fontSize: "16px", fontWeight: 700, marginBottom: "4px" }}>{result.bestPlatform}</p>
          ...
          {/* Listing - Facebook/Kijiji */}
          <div className="card">
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "10px" }}>
              <p className="card-label" style={{ marginBottom: 0 }}>For Facebook / Kijiji</p>
              <button onClick={() => cp(`${result.title}\n\n${result.description}`, "short")} className={`btn-copy ${copied === "short" ? "copied" : ""}`}>{copied === "short" ? "Copied!" : "Copy All"}</button>
            </div>
            <p style={{ fontSize: "15px", fontWeight: 600, lineHeight: 1.4, marginBottom: "8px" }}>{result.title}</p>
            <p style={{ fontSize: "14px", lineHeight: 1.6, color: "var(--text-secondary)" }}>{result.description}</p>
          </div>
          ...
          {(result.ebayTitle || result.ebayDescription) && (
            <div className="card">
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "10px" }}>
                <p className="card-label" style={{ marginBottom: 0 }}>For eBay</p>
                <button onClick={() => cp(`${result.ebayTitle || result.title}\n\n${result.ebayDescription || result.description}`, "ebay")} className={`btn-copy ${copied === "ebay" ? "copied" : ""}`}>{copied === "ebay" ? "Copied!" : "Copy All"}</button>
              </div>
              <p style={{ fontSize: "15px", fontWeight: 600, lineHeight: 1.4, marginBottom: "8px" }}>{result.ebayTitle || result.title}</p>
              <p style={{ fontSize: "14px", lineHeight: 1.6, color: "var(--text-secondary)" }}>{result.ebayDescription || result.description}</p>
            </div>
          )}
